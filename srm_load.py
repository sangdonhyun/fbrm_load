...
class srm_load():
    ...
    def del_tablespace(self,dict_list,oracle_sid):
        query = "select instance_name,file_name from live.live_ora_tablespace where db_name = '{}'".format(oracle_sid)
        ret = self.db.get_row(query)

        query = "delete from live.live_ora_tablespace where db_name = '{}'".format(oracle_sid)
    ...
    def set_table_space(self,tbs_list,dbname):
        """
        ...
        for tbs in tbs_list:
            ...
            ins_dict['db_name'] = dbname.strip()
            ins_dict_list.append(ins_dict)



        tb = 'live.live_ora_tablespace'
        query = "delete from {} where instance_name = '{}'".format(tb, self.oracle_sid)
        query = "delete from {} where instance_name = '{}'".format(tb, self.oracle_sid)
        self.db.queryExec(query)
    ...
    def set_parameter(self,param_list):
        ...
        for param in param_list:
            ...
            print 'param :',param[4]
            ins_dict['description'] = str(param[5].strip())
            ins_dict_list.append(ins_dict)
        tb_name = 'live.live_ora_parameter'
        query = "delete from {} where instance_name = '{}' ".format(tb_name, self.oracle_sid,self.fbrm_date)
        print query
        self.db.queryExec(query)
        self.db.dbInsertList(ins_dict_list, tb_name)
    ...
    def set_tempdb(self,tempdb_list):
        ...
        query = "delete from {} where instance_name = '{}' ".format(tb_name,self.oracle_sid)
        print query
        self.db.queryExec(query)
    ...
    def filesystem(self,data):
        ...
        for i in range(len(lineset)):
            ...
            if arg_set[0] == 'FNPSPARM':
                ...
                if not instance_name == '':
                    instance_name=arg_set[1]
            if arg_set[0] == 'DBNAME':
                dbname = arg_set[1]

            if re.match('^Real Application Clusters',line):
                if ',' in line:
                    self.rac_mode = line.split(',')[1].strip()



        self.set_table_space(tb_space_list,dbname)
        try:
        ...
        query = "UPDATE live.live_ora_tablespace	SET  db_name='{db_name}'	where instance_name = '{sid}' ".format(db_name=dbname,sid=self.oracle_sid)
        self.db.queryExec(query)
        query = "UPDATE store.store_day_ora_tablespace	SET  db_name='{db_name}'	where instance_name = '{sid}' ".format(
            db_name=dbname, sid=self.oracle_sid)
        self.db.queryExec(query)

        # query = "UPDATE master.master_ora_info	SET  db_name='{db_name}'	where ora_sid = '{sid}' ".format(
        #     db_name=dbname, sid=self.oracle_sid)
        self.db.queryExec(query)

    def get_db_name(self,fead):
        db_name = ''
        for line in fead.splitlines():
...
FROM master.master_ora_info;
        ...
        query = "select svr_id from master.master_svr_info where svr_hostname = '{hostname}' and svr_ip_v4 = '{agent_ip}'".format(hostname=self.hostname, agent_ip=self.agent_ip)
        print query
        ret = self.db.get_row(query)[0][0]
        if not ret==None:
            svr_serial = self.db.get_row(query)[0][0]
        else:
            svr_serial = ''
    ...
    def main(self):
        ...
        self.set_master_serial()
    def test(self):
        mster_dict = {'ora_is_rac': 'FALSE', 'inst_datetime': '2020-10-21 12:36:54', 'svr_hostname': 'ibrmsv01', 'svr_id': 8004,
          'fbrm_datetime': '2020-10-21 12:36:54', 'db_name': 'ibrm', 'ora_sid': 'ibrm',
          'svr_ip_v4': '121.170.193.200', 'ora_home': '/u01/app/oracle/product/18.0.0/dbhome_1'}
        tb_name = 'master.master_ora_info'
        print mster_dict
        print tb_name
        self.db.db_master_insert(mster_dict, tb_name)


    def set_master_serial(self):
        query = 'select asn,peer_asn from master.master_zfs_cluster'
        ret=self.db.get_row(query)
        serial_list=[]
        serial = ''
        for row in ret:
            ...
            # query ="UPDATE master.master_zfs_info	set zfs_serial='{}'	WHERE asn='{}';".format(serial,row[0])
            # self.db.queryExec(query)
            # query = "UPDATE master.master_zfs_cluster	set zfs_serial='{}'	WHERE asn='{}';".format(serial, row[0])
            # print query
            # self.db.queryExec(query)
